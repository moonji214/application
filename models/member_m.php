...
class Member_m extends CI_Model
{
	...
	public function login($data = array())
	{
		...
		$this->db->select('id, u_name, pw, u_phone, u_email , u_admin');
		$query = $this->db->get_where('tb_member', array('id' => $data['id'] , 'pw' => md5($data['pw']))  );
		
		if ( ! empty($query) && $query->num_rows() > 0 ) {
			$return = $query->row_array();
			$return['result'] = TRUE;	
		} else {
			$return = array('result' => FALSE, 'message' => '계정이 존재하지 않습니다.');
		}
